...
def buscar_informacoes(valor_busca: str) -> dict:
    ...
    df_tuss[col_procedimento] = df_tuss[col_procedimento].astype(str)
    ...
    # 3) Verifica se o valor está em 'procedimento'
    if valor_busca in df_tuss[col_procedimento].values:
        linha = df_tuss.loc[df_tuss[col_procedimento] == valor_busca].iloc[0]
        return {
            col_codigo: linha[col_codigo],
            col_tuss: linha[col_tuss],
            col_procedimento: linha[col_procedimento]
